...
assessmentButton.onclick = () => {
     //onclickと設定すれば、ボタンが押された時に関数が実行される
const userName = userNameInput.value ;
if (userName.lenght === 0){
    //名前が空の時は処理を終了する
    return;
    //関数の処理の中でreturnと書くと、戻り値なしに処理が終了する。の意味となる。
}
...
/**
 * 名前の文字列を渡すと診断結果を返す関数
 * ＠param{string}userName　ユーザーの名前 parameter:引数
 * ＠return{string}診断結果　return:戻り値　string:文字列
 */
...
result = result.replace(/\{userName\}/g,userName);
         //\{userName\}gを全て選択し、userNameの項目に変更する意味。
return result;
}
//テストコード
...
console.assert(
    assessment('太郎') ===
    '太郎のいいいところは決断力です。太郎がする決断にいつも助けられる人がいます。',
